#include "ForcePredAsAssumePass.hpp"
...
/*
 * STEPS:
 * (1) Run each basic block
 * (2) Run each loop block
 * (3) Get the loop predicate
 * (4) Identify getExitBB, i.e., exit basic block
 * (5) Add in after the loop block the function __VERIFIER_assume with the 
 *     negation the predicate got in the step 3 
 * */

bool ForcePredAsAssumePass::runOnFunction(Function& F) {
...
char ForcePredAsAssumePass::ID = 13;
static RegisterPass<TrackBasicBlockPass> X(
    "force_pred_as_assume", "Force loop predicate as assume in the code");
